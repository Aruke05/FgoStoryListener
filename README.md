...
# 生成 dist-v2.9.5\FgoStoryListener
.\build.ps1
```
...
- 设置会列出 Codex 自己的模型缓存。若其他电脑仍安装旧版 CLI，新模型可能被服务端要求升级；软件会保守回退到已验证的 `gpt-5.5`。
- 如果实时常驻模型被当前 `codex-cli` 拒绝，服务端返回“requires a newer version of Codex”时，实时配置会自动切换到兼容的 `gpt-5.5` 并原批重试。预翻译不会静默降级 Sol/high；失败内容保留在数据库，可用“重试未翻译”重新加入独立的预翻译队列。
